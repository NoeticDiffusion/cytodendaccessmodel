"""Experiment 04 — Cross-plane coupling replication: sub-656228 (DANDI 000871).
...
Subject file mapping (confirmed by probe):
  sub-656228_ses-1247233186-acq-1247385128  →  shallow plane, 4 ROIs
  sub-656228_ses-1247233186-acq-1247385130  →  deep plane,    25 ROIs
...
Usage:
    python experiments/dandi_000871_04_sub656228_replication.py

Outputs:
    data/dandi/triage/000871/crossplane_coupling_sub656228.json
    data/dandi/triage/000871/crossplane_coupling_sub656228.md
    data/dandi/triage/000871/crossplane_coupling_sub656228.log
"""
...
_SHALLOW_REL = f"{_SUBJECT}/{_SUBJECT}_{_SESSION}-acq-1247385128_image+ophys.nwb"
_DEEP_REL    = f"{_SUBJECT}/{_SUBJECT}_{_SESSION}-acq-1247385130_image+ophys.nwb"
...
# ---------------------------------------------------------------------------
# Data loading — identical to exp03
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Window helpers — identical to exp03
# ---------------------------------------------------------------------------
...
def _write_summary(path: Path, results: dict, subject: str) -> None:
    lines = [
        f"# Cross-plane Coupling Summary: DANDI 000871 — {subject}",
        "",
...
def main() -> None:
    ...
    _emit(log_lines, "=" * 60)
    _emit(log_lines, f"Exp 04 — Cross-plane Coupling: DANDI 000871 ({_SUBJECT})")
    ...
    if "spontaneous" in results:
        ...
        if crs > 0 and z > 1.96:
            if crs < min(wsh, wde):
                verdict = "H3 POSITIVE — cross-plane above null AND below within-plane (access-constraint signature)"
            else:
    ...
    if ref_json.exists():
        ...
        _emit(log_lines, "  (sub-644972 reference JSON not found — run exp03 first)")
