...
// Tables in this appendix are numbered S3.1, S3.2, …
...
The `000718` analysis @Zaki2024DANDI000718 targets a narrow question: are the units most central to a NeutralExposure ensemble preferentially reactivated during later high-synchrony offline events?
...
#figure(
  kind: table,
  supplement: [Table],
  caption: [Compact robustness summary for the DANDI `000718` offline core-unit enrichment analysis.],
  table(
...
The `000336` analysis @Amaya2026DANDI000336 targets a different question: do paired imaging planes communicate above null while remaining less coupled than units within the same plane?
...
#figure(
  kind: table,
  supplement: [Table],
  caption: [Compact robustness and claim-boundary summary for the DANDI `000336` full-bundle coupling analysis.],
  table(
...
The `001710` analysis @Plitt2026DANDI001710 targets a third question: do longitudinal hippocampal population codes show the combination of stable spatial tuning, within-day arm separation, and partial cross-day continuity expected from a context-sensitive accessibility account, and does perturbing a candidate postsynaptic structural-write mechanism reduce that continuity selectively across days?
...
#figure(
  kind: table,
  supplement: [Table],
  caption: [Compact QC summary for the broadened DANDI `001710` genotype bundle, highlighting group coverage, ROI ranges, and split-half reliability before claim interpretation.],
  table(
...
#figure(
  kind: table,
  supplement: [Table],
  caption: [Subject-level permutation null tests for the broadened DANDI `001710` genotype comparison. SparseKO falls below Cre under the implemented subject-level null and is directionally lower than Ctrl, but the latter contrast is weaker.],
  table(
...
) <tab-s3-dandi1710-null>

#figure(
  kind: table,
  supplement: [Table],
  caption: [SparseKO channel comparison for the broadened DANDI `001710` robustness pass. The two channels differ quantitatively but not qualitatively: both support preserved within-session structure, while channel `1` is somewhat more stable across days on average.],
  table(
    columns: (auto, auto, auto, auto, auto),
    inset: 6pt,
    stroke: 0.5pt + black,
...
) <tab-s3-dandi1710-channels>

#figure(
  kind: table,
  supplement: [Table],
  caption: [Group-level day-lag profile for the broadened DANDI `001710` robustness pass. SparseKO remains lower than the non-KO cohorts across all tested day-lags, arguing against the stability deficit being driven by a single recording interval.],
  table(
    columns: (auto, auto, auto, auto, auto, auto),
    inset: 6pt,
    stroke: 0.5pt + black,
